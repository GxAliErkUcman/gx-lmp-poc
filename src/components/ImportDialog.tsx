...
const ImportDialog = ({ open, onOpenChange, onSuccess }: ImportDialogProps) => {
  ...
  const importData = async () => {
    ...
    setLoading(true);
    try {
      // First, get the user's profile to get their client_id (optional)
      ...
      // If profile is missing, continue without client_id (RLS allows NULL)
      ...
      const businessesToInsert = parsedData.map(row => {
        ...
        if (profile?.client_id) {
          business.client_id = profile.client_id;
        }
