-- Step 1: Drop ALL existing recursive policies on businesses
...
-- Step 2: Create SECURITY DEFINER function for store owner access (prevents recursion)
...
-- Step 3: Create separate SELECT policies (5 policies - no overlap, no recursion)
...
-- Step 7: Fix store_owner_access policies to prevent recursion
DROP POLICY IF EXISTS "Client admins can manage store owner access in their client" ON public.store_owner_access;

-- Create SECURITY DEFINER function for managing store owner access
