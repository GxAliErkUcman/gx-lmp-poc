...
const singleUrlPattern = /^$|^https?:\/\/([-A-ÿ0-9ZŠĐČĆŽzšđčćž@:%._\+~#=]{1,256}\.[A-ÿ0-9ZŠĐČĆŽzšđčćž()]{1,6}\b(?:[-A-ÿ0-9ZŠĐČĆŽzšđčćž()@:%_\+.~#?&\/=]*))$/;
const multipleUrlsPattern = /^$|^https?:\/\/([-A-ÿ0-9ZŠĐČĆŽzšđčćž@:%._\+~#=]{1,256}\.[A-ÿ0-9ZŠĐČĆŽzšđčćž()]{1,6}\b(?:[-A-ÿ0-9ZŠĐČĆŽzšđčćž()@:%_\+.~#?&\/=]*))((\s)*,(\s)*https?:\/\/([-A-ÿ0-9ZŠĐČĆŽzšđčćž@:%._\+~#=]{1,256}\.[A-ÿ0-9ZŠĐČĆŽzšđčćž()]{1,6}\b(?:[-A-ÿ0-9ZŠĐČĆŽzšđčćž()@:%_\+.~#?&\/=]*))){0,}$/;
...
const GOLDMINE_ENABLED_CLIENTS = [
  '75d14738-25d0-4c40-9921-bde980bc8e06',
  '3c9ceb08-13f9-4f50-9945-857e3770940f'
];

function parseGoldmine(goldmine: string): Record<string, string> {
  const result: Record<string, string> = {};
  if (!goldmine || !goldmine.trim()) return result;
...
function convertToJsonSchema(business: Business & { goldmine?: string }, includeGoldmine: boolean = false) {
  ...
  if (includeGoldmine && business.goldmine) {
    ...
    return { ...baseSchema, ...goldmineFields };
  }
  return baseSchema;
}

// Helper to log execution result
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (clientError) {
      ...
      await logExecution(supabase, 'generate-json-export', 'error', requestBody, startTime, `Failed to fetch client: ${clientError.message}`);
    ...
    if (error) {
      ...
      await logExecution(supabase, 'generate-json-export', 'error', requestBody, startTime, `Failed to fetch businesses: ${error.message}`);
    ...
    if (uploadError) {
      ...
      await logExecution(supabase, 'generate-json-export', 'error', requestBody, startTime, `Failed to upload export: ${uploadError.message}`);
    ...
    // Auto-sync with GCP
    let gcpSyncError: string | null = null;
    try {
      ...
      if (syncResult.success) {
        ...
        gcpSyncError = syncResult.error || 'Unknown GCP sync error';
        console.warn('GCP sync failed:', gcpSyncError);
      }
    } catch (syncError) {
      gcpSyncError = (syncError as any)?.message || String(syncError);
      console.warn('GCP sync error (non-fatal):', gcpSyncError);
    }
    ...
    // Log success (but note if GCP sync failed)
    if (gcpSyncError) {
      await logExecution(supabase, 'generate-json-export', 'error', requestBody, startTime, `Export OK but GCP sync failed: ${gcpSyncError}`, responseBody);
    } else {
      await logExecution(supabase, 'generate-json-export', 'success', requestBody, startTime, undefined, responseBody);
    }
    ...
    try {
      ...
      await logExecution(supabase, 'generate-json-export', 'error', requestBody, startTime, msg);
